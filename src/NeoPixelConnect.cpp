...
NeoPixelConnect::NeoPixelConnect(byte pinNumber, byte numberOfPixels, PIO pio, uint sm)
{
    this->pixelSm = sm;
    this->pixelPio = pio;
    this->neoPixelInit(pinNumber, numberOfPixels);
...
/// @param gammaCorrecct: If true, apply gamma correction.
/// @param autoShow: If true, show the change immediately.
void NeoPixelConnect::setPixel(uint8_t pixelNumber, uint8_t r, uint8_t g, uint8_t b, bool gammaCorrect, bool autoShow)
{
    this->pixelBuffer[pixelNumber][RED] = gammaCorrect ? gamma8(r) : r;
    this->pixelBuffer[pixelNumber][GREEN] = gammaCorrect ? gamma8(g) : g;
    this->pixelBuffer[pixelNumber][BLUE] = gammaCorrect ? gamma8(b) : b;
    if (autoShow) {
        this->show();
    }
}
...
/// @param autoShow: If true, show the change immediately.
void NeoPixelConnect::setPixelValue(uint8_t pixelNumber, color_t color, uint8_t val, bool gammaCorrect, bool autoShow)
{
    this->pixelBuffer[pixelNumber][color] = gammaCorrect ? gamma8(val) : val;
    if (autoShow) {
        this->show();
    }
}
...
/// @param color: RED, GREEN, or BLUE
uint8_t NeoPixelConnect::getPixelValue(uint8_t pixelNumber, color_t color)
{
    return this->pixelBuffer[pixelNumber][color];
}
